<template>
  <div :class="CSS_PREFIX" v-clickoutside="handleclickoutside" @paste="doPaste">
    <div :class="`${CSS_PREFIX}-main`">
      ...
      <div :class="`${CSS_PREFIX}-overlayer`" :style="{ top: `${tableHeaderHeight + 1}px` }">
        <div :class="`${CSS_PREFIX}-editor`" ref="editorRef" :style="editorSty">
          <div ref="textRef" contenteditable="true" v-show="isSimple" @input="inputHandler" @keydown.tab.prevent
            @keydown.enter="handleEnter" @keydown.esc.prevent>
          </div>
          <el-date-picker ref="monthRef" v-if="dataType === 'month'" :class="`${CSS_PREFIX}-popup`" :style="popupSty"
            v-model="value" :editable="false" type="month" size="small" placeholder="选择月份" format="yyyy-MM"
            value-format="yyyy-MM" @change="selectChange">
          </el-date-picker>
          <el-date-picker ref="dateRef" v-else-if="dataType === 'date'" :class="`${CSS_PREFIX}-popup`" :style="popupSty"
            v-model="value" :editable="false" type="date" size="small" placeholder="选择日期" format="yyyy-MM-dd"
            value-format="yyyy-MM-dd" @change="selectChange">
          </el-date-picker>
          <el-date-picker ref="datetimeRef" v-else-if="dataType === 'datetime'" :class="`${CSS_PREFIX}-popup`"
            :style="popupSty" v-model="value" :editable="false" type="datetime" size="small" placeholder="选择日期时间"
            format="yyyy-MM-dd HH:mm" value-format="yyyy-MM-dd HH:mm" @change="selectChange">
          </el-date-picker>
          <el-select ref="selectRef" v-else-if="dataType === 'select'" :class="`${CSS_PREFIX}-popup`" :style="popupSty"
            v-model="value" clearable filterable size="small" :automatic-dropdown="true" @change="selectChange">
...
import { ref, computed, nextTick, watch, onUnmounted, getCurrentInstance, ComponentInternalInstance } from 'vue';
import { CSS_PREFIX as cssPrefix, HEADER_HEIGHT } from "../../core/constants.js";
...
watch(props.columns, (newValue: Array<any>) => {
  ...
  grid.value.loadData(props.data);
});
...
function editCell() {
  ...
  textRef.value.innerHTML = focusValue;
  grid.value.setTempData(value)
  if (focusDataType === "month" || focusDataType === "date" || focusDataType === "datetime") {
...
function showEditor() {
  ...
  editorRef.value.style.left = `${x - 1}px`;
  editorRef.value.style.top = `${y - 2 - tableHeaderHeight.value}px`;
  textRef.value.style["min-width"] = `${width - 1}px`;
  textRef.value.style["min-height"] = `${height - 1}px`;
...
function hideEditor() {
  ...
  editorRef.value.style.left = "-10000px";
  editorRef.value.style.top = "-10000px";
  show.value = false;
  dataType.value = "textRef";
};
